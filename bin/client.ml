...
let initiate () =
  ...
  while isConnected() do
      let t1 = Thread.create (Util.handle_receive_messages client_socket "server" onDisconnected) () in
      let t2 = Thread.create (Util.handle_send_messages (dup client_socket) "client" isConnected) () in
      Thread.join t1;
      Thread.join t2;
  done;
